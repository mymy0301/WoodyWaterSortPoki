      ...
      _export("resourceUtil", resourceUtil = (_dec = ccclass("resourceUtil"), _dec(_class = class resourceUtil {
        ...
        static loadSpriteFrameRes(path, sprite, cb) {
          ...
          return new Promise((resolve, reject) => {
            ...
            var bundel = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).BUNDLE_ITEMS;
            var ret = assetManager.getBundle(bundel).get(path, SpriteFrame);
        ...
        static loadPrefabUI_Bundle(uiPopupName, cb) {
          ...
          return new Promise((resolve, reject) => {
            var path = 'prefabsUI/' + uiPopupName;
            var bundel = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).BUNDLE_ITEMS;
            var ret = assetManager.getBundle(bundel).get(path, Prefab); // console.log(path);    
            // console.log(bundel);    
            // console.log(ret);    
        ...
        static loadPrefabCat_Bundle(pathCat, cb) {
          return new Promise((resolve, reject) => {
            var path = 'cat/cat' + pathCat;
            var bundel = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
              error: Error()
            }), Constants) : Constants).BUNDLE_ITEMS;
            var ret = assetManager.getBundle(bundel).get(path, Prefab);
          ...
          return new Promise((resolve, reject) => {
            ...
            assetManager.downloader.downloadDomImage(url, {
              ...
              if (err) {
                ...
                var imageAsset = new ImageAsset(res);
