    ...
    execute: function () {
      ...
      _export("BoxRewardInfoGroup", BoxRewardInfoGroup = (_dec = ccclass('BoxRewardInfoGroup'), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Node), _dec5 = property(UITransform), _dec(_class = (_class2 = (_temp = class BoxRewardInfoGroup extends Component {
        ...
        showBoxRewardInfoGroup(posStart, isBottom, groupRewardDataInfo) {
          ...
          var sizeX = groupRewardDataInfo.items.length * 90 + (groupRewardDataInfo.items.length - 1) * 30;
          var sizeBGX = sizeX + 40;
          var posX = (sizeBGX - 215) / 2;
          ...
          for (var i = 0; i < groupRewardDataInfo.items.length; i++) {
            var item = instantiate(this.getItemRewardReceivePrefab(groupRewardDataInfo.items[i].itemShopType));
            item.parent = this.contentGroup;
            var itemBoxReward = item.getComponent(_crd && ItemBoxReward === void 0 ? (_reportPossibleCrUseOfItemBoxReward({
              error: Error()
            ...
            if (i < groupRewardDataInfo.items.length - 1) {
              var _item = instantiate(this.itemPlus);

              _item.parent = this.contentGroup;
            }
          }
