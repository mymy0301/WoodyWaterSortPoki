    ...
    execute: function () {
      ...
      _export("default", Utils = class Utils {
        ...
        */
        ...
        static getClockStrByS(second, showsecond = true, showhour = true) {
          second = Math.floor(second);
          if (second < 0) second = 0;
          var h = Math.floor(second / 3600);
          second -= h * 3600;
        ...
        static loadRes(path, type, callback = null) {
          return new Promise((resolve, reject) => {
        ...
        static async playSkAni(file, name, parent, pos, removetime = -1) {
          var node = new Node();
          node.parent = parent;
          node.position = pos;
          node.layer = Layers.BitMask.UI_2D;
          var skd = node.addComponent(sp.Skeleton);
          var data = await Utils.loadRes(file, sp.SkeletonData);
        ...
        static getPolygonIntersection(point, angle, polygon) {
          let intersections = []; // Chuyển đổi góc từ độ sang radian

          const angleRad = angle * (Math.PI / 180); // Xác định hệ số góc của đường thẳng

          const slope = Math.tan(angleRad); // Tạo một đối tượng điểm đầu và điểm cuối của đoạn thẳng dài đủ để cắt toàn bộ đa giác

          const lineStart = new Vec2(point.x - 1000, point.y - 1000 * slope);
          const lineEnd = new Vec2(point.x + 1000, point.y + 1000 * slope); // Duyệt qua từng cạnh của đa giác
